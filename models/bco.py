...
class BCO():
    ...
    def encode(self, state, next_state):
        ...
        with tf.variable_scope("bc_action", reuse=True):
            with tf.variable_scope("state_encoding", reuse=True):
                return self.create_encoder(tf.concat([state, next_state], axis=-1))

    def create_bc(self, s_t, s_prime, generator_outputs_channels):
        """Creates the Behavioral Cloning Network."""

        # Create state embeddings.
        with tf.variable_scope("state_encoding"):
            s_t_layers = self.create_encoder(tf.concat([s_t, s_prime], axis=-1))
    ...
    def create_model(self, inputs, actions, targets):
        """ Initializes BC model and losses."""
        actions = tf.one_hot(actions, args.n_actions)
        global_step = tf.contrib.framework.get_or_create_global_step()
        incr_global_step = tf.assign(global_step, global_step+1)

        with tf.variable_scope("bc_action"):
            out_channels = int(inputs.get_shape()[-1])
            predicted_actions = self.create_bc(inputs, targets, out_channels)

        with tf.name_scope("bc_loss"):
            bc_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=actions, logits=predicted_actions))
